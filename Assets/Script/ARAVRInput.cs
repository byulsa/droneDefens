...
# endif
    ...
    {
#if PC
        // PC 테스트 환경: 키보드로 왼쪽/오른쪽 스틱 분리
        if (hand == Controller.LTouch)
        {
            // 왼쪽 스틱: 위/아래(W, S), 좌/우(A, D)
            if (axis == "Horizontal") return Input.GetKey(KeyCode.D) ? 1f : (Input.GetKey(KeyCode.A) ? -1f : 0f);
            else return Input.GetKey(KeyCode.W) ? 1f : (Input.GetKey(KeyCode.S) ? -1f : 0f);
        }
        else
        {
            // 오른쪽 스틱: 위/아래(Up, Down 방향키), 좌/우(Left, Right 방향키)
            if (axis == "Horizontal") return Input.GetKey(KeyCode.RightArrow) ? 1f : (Input.GetKey(KeyCode.LeftArrow) ? -1f : 0f);
            else return Input.GetKey(KeyCode.UpArrow) ? 1f : (Input.GetKey(KeyCode.DownArrow) ? -1f : 0f);
        }
#elif Oculus
    ...
    // ��Ʈ�ѷ��� ���� ȣ���ϱ�
    // duration: ���� �ð�, frequency: ��, amplify: ����, hand: ���� Ȥ�� ������ ��Ʈ�ѷ�
